...
class SmartHumidityGUI:
    ...
    def setup_ui(self):
        ...
        # --- Current humidity slider (dummy command until widgets exist) ---
        ...
        self.hum_scale = ttk.Scale(
            ...
            command=lambda _: None, length=200
        )
        ...
        # --- Target humidity slider (dummy command until widgets exist) ---
        ...
        # --- Now that labels exist, wire real callbacks ---
        self.hum_scale.configure(command=self.update_display)
        self.target_scale.configure(command=self.update_setpoint)
    ...
    def create_mist_particles(self, device_type, power):
        ...
        for _ in range(num_particles):
            ...
            if device_type == "humidifier":
                ...
                vx, vy = random.uniform(-0.5, 0.5), random.uniform(0.5, 2)
                color = "#e67e22"
    ...
    def update_display(self, _=None):
        ...
        if action < -10:
            ...
            self.canvas.itemconfig(self.device, fill="#e67e22")
            self.canvas.itemconfig(self.power_indicator, fill="#d35400")
            ...
            self.canvas.itemconfig(self.device, fill="#3498db")
            self.canvas.itemconfig(self.power_indicator, fill="#2980b9")
            self.create_mist_particles(device_type, power)

        else:
            dev_text = "Both OFF"
            self.canvas.itemconfig(self.device, fill="#95a5a6")
            self.canvas.itemconfig(self.power_indicator, fill="#7f8c8d")
